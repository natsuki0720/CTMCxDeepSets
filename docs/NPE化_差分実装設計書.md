...
### 10.7 K_REF: 弱識別データセットの表現可能性(2026-06-12 追加)

§10.3 の `_inv_sqrt_k` は当初 `exp(−0.5·logK)=K^{-1/2}`、すなわち残差を **K=1 基準**で
標準化していた。識別良好なら `SD(u)≈O(1)〜数十` で問題ないが、**弱識別データセット**
(ある状態の訪問サンプルが僅少。初期分布が Dirichlet(1,1,1) で単体の端に質量を持つため
訓練分布に確実に存在)では事後≈事前で `SD(z−μ)≈1.33`、よって `SD(u)=1.33·√K≈94`(K=5000)。
これは **Gaussian の log_diag clamp 上限 e³≈20 も NSF spline bound(±5)も超過し表現不能**。
しかも実データで最も見せたい「事後が広がるべき領域」がここ。

**修正(実質1行):** 残差を **K=k_ref(既定1000)基準**で標準化する。

```python
# _inv_sqrt_k:  sqrt(k_ref / K)
return torch.exp(-0.5 * (logk - math.log(self.k_ref))).unsqueeze(-1)
```

`k_ref` は定数なので **1/√K 収縮・複製比 1/√2・傾き −1/2 は不変**。弱識別でも
`SD(u)=1.33·√(K/1000)≤3`(K=5000)と clamp・bound 内に収まる。`k_ref` は model_config に
保存し predictor 再構築まで伝播。併せて `fit()` に勾配クリップ(`--grad-clip` 既定5.0)を追加
(大残差バッチの NLL スパイク対策)。

**検証(8000セット, ゲートrun `struct_gauss3`):** 複製比 0.714 / 帯内傾き
[−0.485,−0.484,−0.502] / 全体被覆 0.534・0.917 — 全ゲート通過。**弱識別層(訪問数<50)の
90%被覆**を before/after で比較:

| | k_ref=1(旧) | k_ref=1000(新) |
|---|---|---|
| 90%被覆 | 0.823(過信) | **0.935** |
| 50%被覆 | 0.435 | 0.387 |

K_REF が弱識別の裾被覆を 0.82→0.94 に適正化(レビュー予測どおり)。50%被覆が両方で
~0.39 と低いのはガウスが弱識別の非ガウス形状を表現しきれない残差で、Phase 2(flow)の領域。

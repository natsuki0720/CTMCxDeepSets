...
統計モデルのパラメータ推定(e.g. 最尤推定，ベイズ推定)は，必ずしも解析解が得られるわけではなく，一般に複雑な反復計算アルゴリズムを要する最適化計算によって近似的な数値解が与えられる．これらで用いられる計算アルゴリズムはその複雑さや，計算コストがしばしば課題となる．  
そこで，統計モデルの推定過程におけるアルゴリズムの複雑さやその計算コストを回避するための方法論を本リポジトリでは提案する．  
本リポジトリでは，CTMCの最尤推定を題材として深層学習モデルを構築するが，提案する方法論はデータセットとそのデータセットを生成する統計モデルのパラメータのペアからなるデータを大量に生成することができれば，任意の統計モデルへの適用が可能である．  
提案手法は，推定のための複雑なアルゴリズムをブラックボックス化することで，難解なアルゴリズムの厳密な定式化・コーディングを回避するとともに，高コストな計算コストを省力化する手法になりうる．  
...
## まとめと展望
今回は**CTMCに対する最尤推定過程**に対し，異なる条件で生成した仮想データを用いて二つのモデルを作成した．CTMCは，社会インフラの統計的劣化モデルのほか，金融，医療といった多様な文脈で用いられる代表的な確率過程であった．  
本リポジトリで提案する最尤推定の代理手法は，精度等に一部課題が残るものの，今後の改善によっては充分実用に耐えるものになると考えられる．訓練コストこそ掛かるものの，推定の即時性や反復性が求められる状況下において，本リポジトリで提案する手法は一定の適用可能性が見込める．  

また，本リポジトリではCTMCの最尤推定という枠組みで代理モデリングを行ったが，本手法はCTMCに限らず，多様な統計モデルの推定に転用可能な方法である．入力となるデータ集合の空間と対応パラメータというデータ構造を，空間に対して充分網羅性を持つ形で仮想データが作成できれば，あらゆる統計モデルに対し，最尤推定過程を代理することが可能な方法である．特に，最尤推定においてアルゴリズムやその定式化が複雑化するような統計モデルにおいて，データ生成フローさえ整えれば最適化計算アルゴリズムそのものをブラックボックス化して最尤推定を実施することが可能である点で本リポジトリで提案する方法論は強力であるといえる．  
  
本リポジトリでは，CTMCの最尤推定プロセスの代理手法を提案したが，主に訓練データ生成の方法を起点とした推定精度や適用範囲に対する課題が生じた．  
これらの課題に対し，以下を追加の検討事項として今後の研究活動に繋げたい．  
- **期待寿命分布を一様分布にしたモデル設計（モデル1に準ずる予定）**
- **点検間隔分布に制約を受けないモデル構築・訓練データ生成手法の考案**
- ベイズ推定への拡張
- CTMC以外のより高度なモデルへの方法論の転用
